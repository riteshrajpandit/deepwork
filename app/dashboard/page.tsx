"use client";
import { Edit, CheckSquare, Calendar, ArrowRight, Folder, Target, Clock } from "lucide-react";
import { useAppContext, formatFriendlyDate } from "@/components/AppProvider";
import Link from "next/link";
import Image from "next/image";

export default function Dashboard() {
  const { projects, tasks, setTaskModalOpen, users } = useAppContext();

  // Pick top 3 projects for "Active Initiatives"
  const activeProjects = projects.filter(p => p.status === 'Active');
  const topProjects = activeProjects.slice(0, 3);
  
  // Pick today's unfinished tasks linked to valid projects
  const focusTasks = tasks.filter(t => t.status !== 'Done' && activeProjects.some(p => p.id === t.projectId)).slice(0, 3);
    ...
    <>
      ...
      <div className="grid grid-cols-1 md:grid-cols-12 gap-6 auto-rows-min">
        ...
        <div className="md:col-span-8 bg-surface-container-lowest border border-outline-variant/50 rounded-xl p-6 ambient-shadow transition-all duration-300 hover:shadow-lg flex flex-col">
          ...
          <div className="space-y-6 flex-1">
            {topProjects.map((project, idx) => {
              ...
              const compTasks = pTasks.filter(t => t.status === 'Done').length;
              const progress = pTasks.length ? Math.round((compTasks / pTasks.length) * 100) : 0;
              const colorClass = idx === 0 ? 'bg-primary' : idx === 1 ? 'bg-secondary' : 'bg-surface-tint';

              return (
                <div key={project.id} className="group">
                  <div className="flex justify-between items-baseline mb-2">
                    <Link href={`/projects/${project.id}`}>
                      <h4 className="text-body-md font-body-md font-medium text-on-surface group-hover:text-primary transition-colors cursor-pointer">{project.name}</h4>
                    </Link>
                    <span className="text-label-sm font-label-sm text-on-surface-variant">{progress}%</span>
                  </div>
                  <div className="h-1.5 w-full bg-surface-container rounded-full overflow-hidden">
                    <div className={`h-full ${colorClass} rounded-full transition-all duration-500`} style={{ width: `${progress}%` }}></div>
                  </div>
                  <p className="mt-2 text-label-sm font-label-sm text-outline">{progress === 100 ? 'All caught up' : `${pTasks.length - compTasks} tasks pending`}</p>
                </div>
              )
            })}
        ...
        <div className="md:col-span-4 bg-surface-container-low border border-surface-container rounded-xl p-6 flex flex-col gap-4">
          ...
          <button onClick={() => setTaskModalOpen({ open: true })} className="w-full bg-surface-container-lowest border border-outline-variant/50 hover:border-primary/50 text-on-surface-variant transition-all duration-200 rounded-lg p-4 flex items-center gap-4 ambient-shadow group text-left active:scale-[0.98] cursor-pointer">
        ...
        <div className="md:col-span-4 bg-surface-container-lowest border border-outline-variant/50 rounded-xl p-6 ambient-shadow flex flex-col">
          ...
          <div className="space-y-1 flex-1">
            {focusTasks.map(task => (
              <label key={task.id} className="flex items-start gap-4 p-3 rounded-lg hover:bg-surface transition-colors cursor-pointer group border border-transparent hover:border-outline-variant/30">
                <input type="checkbox" readOnly checked={task.status === 'Done'} className="mt-1 w-5 h-5 rounded border-outline-variant text-primary focus:ring-primary/20 bg-transparent transition-colors" />
                <div>
          ...
          <div className="space-y-4 flex-1">
            ...
            <div className="flex gap-3 group cursor-pointer">
              <div className="flex flex-col items-center mt-1">
                <div className="w-2.5 h-2.5 rounded-full bg-primary ring-4 ring-surface-container-lowest"></div>
            ...
            <div className="flex gap-3 group cursor-pointer">
              <div className="flex flex-col items-center mt-1">
                <div className="w-2.5 h-2.5 rounded-full bg-secondary ring-4 ring-surface-container-lowest"></div>
                <div className="w-px h-full bg-outline-variant/30 mt-1"></div>
              </div>
              <div className="pb-4">
            ...
            <div className="flex gap-3 group cursor-pointer">
              <div className="flex flex-col items-center mt-1">
                 <div className="w-2.5 h-2.5 rounded-full bg-outline-variant ring-4 ring-surface-container-lowest group-hover:bg-primary transition-colors"></div>
              </div>
              <div>
           ...
           <div className="relative pl-4 space-y-6 before:absolute before:inset-y-0 before:left-5 before:w-px before:bg-outline-variant/50 flex-1">
              <div className="relative pl-6">
                ...
                <p className="text-body-md font-body-md text-on-surface leading-snug">
                  <span className="font-medium">Sarah Jenkins</span> completed milestone <span className="font-medium text-primary cursor-pointer hover:underline">Phase 1 Design</span>
                </p>
              </div>
              <div className="relative pl-6">
                <div className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full bg-outline ring-4 ring-surface-container-lowest"></div>
                <p className="text-label-sm font-label-sm text-outline mb-1">Yesterday</p>
                <p className="text-body-md font-body-md text-on-surface leading-snug">
                  <span className="font-medium">You</span> uploaded a new document <span className="font-medium text-primary cursor-pointer hover:underline">Q4 Resource Planning.pdf</span>
                </p>
              </div>
              <div className="relative pl-6">
                <div className="absolute -left-[5px] top-1.5 w-2.5 h-2.5 rounded-full bg-secondary ring-4 ring-surface-container-lowest"></div>
                <p className="text-label-sm font-label-sm text-outline mb-1">Yesterday</p>
                <p className="text-body-md font-body-md text-on-surface leading-snug">
